...
multiwave_diagram <- function(x, height = NULL, width = NULL){
  ...
  if("title" %in% names(x@metadata) &
    ...
    title_char <- paste("rec1 [label = <<b>", x@metadata$title,
  ...
  overall_md_text <-
    ifelse(length(x@metadata) == 0,
           ">",
           paste("<br/> list of length", as.character(length(x@metadata)),
                 ">"))
  p1_md_color <-
  ...
  default_rec <- c(paste("rec2 [label = <<b>Metadata</b>",
                         overall_md_text,
                         overall_md_color),
                   paste("rec3 [label = <<b>Phase 1</b>>,",
                         "fillcolor = burlywood, style = filled]"),
                   paste("rec4 [label = <<b>Metadata</b>",
                         p1_md_text,
                         p1_md_color),
                   paste("rec5 [label = <<b>Data</b>",
                         p1_data_text,
                         p1_data_color)
                   )
  ...
  if (n_phases > 1){
    ...
    for (i in 2:n_phases){
      ...
      for (j in seq_len(n_waves[i])){
        ...
        recs[[(j-1)*6+start+4]] <- paste(samples_rec, samples_text,
                                         samples_color)
        recs[[(j-1)*6+start+5]] <- paste(sampled_data_rec,
                                         sampled_df_text,
                                         sampled_df_color)
  ...
  if (n_phases > 1){
    ...
    for (i in 2:n_phases){
      ...
      edges[[i]] <- paste(phases_recs[i-1],"->", "{",
            paste(recs_vec_short[waves_recs_indices[1] - 1],
                  recs_vec_short[waves_recs_indices], collapse = " "),
            "}")
    }
    ...
    for (i in 2:n_phases){
      ...
      for (j in seq_along(waves_recs_indices)){
        edges[[n_phases + n_waves_prev[i-1] + j]] <-
          paste(recs_vec_short[waves_recs_indices[j]],
                "-> {",
