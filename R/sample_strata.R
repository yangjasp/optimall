...
#' @param wave2a a character sting specifying the name of the column in \code{data1} which indicates (1/0 or Y/N) whether a unit has already been sampled in a prior wave. Defaults to NULL which means that none have been sampled yet.
...
#' @param strata2 a character string specifying the name of the column in \code{data2} which indicates the stratum. Defaults to "strata".
#' @param n_allocated a character string specifying the name of the column in \code{data2} which indicates the n allocated to each stratum. Defaults to "n_to_sample".
#' @export
...
sample_strata <- function(data1, strata1, id, wave2a = NULL, data2, strata2, n_allocated){
  ...
  if (is.null(wave2a) == FALSE){
    if (wave2a %in% names(data1) == FALSE) {
      stop("'wave2a' must be a character string matching a column name of 'data1'.")
      }
    if (length(table(data1[,wave2a])) != 2) {
      stop("'wave2a' must be a character string matching a column in 'data1' that has a binary indicator for whether each unit was already sampled.")
      }
  ...
  if (is.null(wave2a) == FALSE){
    for (i in 1:nrow(data2)){
      stratum <- data2[,strata2][i]
      strata_data <- data1[data1[,strata1] == stratum
                           & data1[,wave2a] != "Y"
                           & data1[,wave2a] != 1,
                           c(id,strata1)]
      sampled_ids[[i]] <- sample(x = strata_data[,id], size = data2[,n_allocated][i])
