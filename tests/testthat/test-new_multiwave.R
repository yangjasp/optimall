...
test_that("initiating with metadata and phase1 data works",{
  test <- new_multiwave(phases = 2, waves = c(1,3),
                        phase1 = data.frame(a = c("a","b","c"),
                                            b = c(1,2,3)),
                        metadata = list(title = "title"))
  expect_equal(get_data(test, phase = 1), data.frame(a = c("a","b","c"),
                                                     b = c(1,2,3)))
  expect_equal(get_data(test, phase = NA, slot = "metadata"),
               list(title = "title"))
...
test_that("errors work",{
  ...
  expect_error(new_multiwave(phases = 2, waves = c(2,3)),
