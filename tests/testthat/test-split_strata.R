...
set.seed(4563)
data_split <- data.frame("strata" = c(rep("a", times = 15),
                                      rep("b", times = 15),
                                      rep("c", times = 12)),
                   "split_var" = c(rnorm(30, sd = 1),
                                   rnorm(12, sd = 2)),
                   "strata2" = rep(c(rep(0, times = 7),
                                     rep(1, times = 7)),
                                   times = 3))
...
test_that("strata_split produces a dataframe with the same number of
  ...
  expect_equal(sort(unique(split_strata(data = data_split,
                                        ...
                                        split_var = "split_var",
                                        split_at = 0,
                                        type = "value")$new_strata)),
               c(paste("a.split_var_(0,",
                       round(max(filter(data_split,
                                        strata == "a")$split_var),
                             digits = 2), "]", sep = ""),
                 paste("a.split_var_[",
                       round(min(filter(data_split,
                                        strata == "a")$split_var),
                             digits = 2),
                       ",0]", sep = ""),
                 "b","c"))
...
test_that("splits occur at correct global quantile values",{
  ...
  expect_equal(substr(median1, start = 7, stop = nchar(median1) - 1 ),
               as.character(round(stats::median(data_split$split_var),
                                  digits = 2)))
  expect_equal(as.vector(table(dplyr::filter(
    split_strata(data = data_split, strata = "strata", split = "a",
                 split_var = "split_var", split_at = 0.5,
                 type = "global quantile"),
    new_strata %in% c("b","c") == FALSE)$new_strata)),
               as.vector(table(dplyr::filter(
                 data_split, strata == "a")$split_var <=
...
test_that("splits occur at correct local quantile values if multiple
  ...
  splitpts <- quantile(dplyr::filter(data_split,
                                     strata == "a")$split_var,
                       c(0.1,0.9))
  ...
  expect_equal(sort(as.vector(table(dplyr::filter(
    split_strata(data = data_split, strata = "strata", split = "a",
                 split_var = "split_var", split_at = c(0.1,0.9),
                 type = "local quantile"),
    new_strata %in% c("b","c") == FALSE)$new_strata)),decreasing = T),
...
test_that("splits occur at correct categorical split",{
  ...
  expect_equal(sort(as.character(unique(
    split_strata(data = data_split, strata = "strata", split = NULL,
                 split_var = "split_var2", split_at = "alpha",
                 type = "categorical")$new_strata))),
    c("a.split_var2_0","a.split_var2_1", "b.split_var2_0",
...
test_that("splits work when multiple strata given to the function",{
  expect_equal(sort(unique(
    split_strata(data = data_split, strata = "strata",
                 ...
                 split_at = 0.5, type = "global quantile")$new_strata)),
    c("a.split_var_(0.03,0.75]", "a.split_var_[-1.71,0.03]",
      "b.split_var_(0.03,1.79]", "b.split_var_[-1.72,0.03]","c"))
  expect_equal(sort(unique(
    split_strata(data = data_split, strata = "strata",
                 split = c("a","b"), split_var = "split_var",
                 split_at = 0.5, type = "local quantile")$new_strata)),
    c("a.split_var_(0.04,0.75]", "a.split_var_[-1.71,0.04]",
...
test_that("strata_split can define prior strata based on an interaction of multiple columns",{
  expect_equal(length(unique(
    split_strata(data = data_split, strata = c("strata","strata2"),
                 ...
                 split_at = 0.5, type = "global quantile")$new_strata)), 7)
})

test_that("when type is a quantile, input must be between 0 and 1 or
          else an error occurs",{
  expect_error(split_strata(data = data_split, strata = "strata",
                            split = "a", split_var = "split_var",
                            split_at = 1.3, type = "global quantile"),
               "'probs' outside [0,1]", fixed = TRUE)
})

test_that("when a 'value' outside of the range of values is given,
          a warning comes up",{
  expect_warning(split_strata(data = data_split, strata = "strata",
                              split = "a", split_var = "split_var",
...
test_that("order is preserved in dataframe with ids provided",{
  ...
  expect_equal(all(split_strata(data = data_split, strata = "strata",
                                split = c("a","b"),
                                split_var = "split_var",
                                split_at = 0.5,
                                type = "local quantile")$split_var ==
...
test_that("truncating the new strata name works properly",{
  expect_equal(sort(unique(
    split_strata(data = data_split, strata = "strata",split = "a",
                 ...
                 type = "value",trunc = "spl")$new_strata)),
    c(paste("a.spl_(0,", round(max(filter(
      data_split, strata == "a")$split_var), digits = 2), "]",
      sep = ""), paste("a.spl_[",
                       round(min(filter(
                         data_split, strata == "a")$split_var),
                         digits = 2), ",0]", sep = ""),  "b","c"))
  expect_equal(sort(unique(
    split_strata(data = data_split, strata = "strata",
                 split = "a",split_var = "split_var",
                 split_at = 0,type = "value",trunc = 4)$new_strata)),
    c(paste("a.spli_(0,", round(max(filter(data_split,
                                           strata == "a")$split_var),
                                digits = 2), "]", sep = ""),
      paste("a.spli_[",
  ...
  expect_equal(sort(unique(
    split_strata(data = data_split, strata = "strata",
                 ...
                 split_at = 0,type = "value",
                 trunc = -2)$new_strata)),
    c(paste("a.ar_(0,", round(max(filter(data_split,
                                         strata == "a")$split_var),
                              digits = 2), "]", sep = ""),
      paste("a.ar_[", round(min(filter(data_split,
                                       strata == "a")$split_var),
                            digits = 2), ",0]", sep = ""),  "b","c"))
  expect_equal(sort(unique(
    split_strata(data = data_split, strata = "strata",
