...
With the number of units to sample per stratum specified in a dataframe, `optimall` can randomly select the IDs of the units to be sampled with the function `sample_strata`:
...
# Created new column called "sample_indicator"
head(iris)
...
Note that `"design_data"` is the output of `optimum_allocation` in this example, but in practice it can be any dataframe that has one row per stratum and one column specifying each stratum's desired sample size.
...
```{r, fig.align='center', echo=FALSE, include = F}
...
`optimall` allows users to input phase-I data and iteratively allocate samples for subsequent waves with the function `allocate_wave`. This function essentially runs integer-valued `optimum_allocation` on a dataset but takes into account previous sampling waves. 
...
wave1_sampled_data <- iris %>%
  dplyr::filter(id %in% wave1_ids) %>%
  dplyr::select(id, Sepal.Width)

wave1_data <- dplyr::left_join(phase1_data, 
                               wave1_sampled_data,
                               by = "id")
...
The simulated data used in this example is included with `optimall`. The dataset `MatWgt_Sim` contains 10335 rows, one for each mother-child pair, and 5 columns containing ID numbers and covariates which we see below:
...
During phase-I, we assume that all 10335 mother-child pairs have been sampled for the inexpensive covariates. Accordingly, we define our phase 1 dataset to be 10335 x 4 with every column of the full `MatWgt_Sim` data excluding the expensive true weight change. In the examples that follow, we will assume the validated maternal weight variable `mat_weight_true` will only be available through phase 2 sampling.
...
#### Conclusion: Assessing the Efficiency of the Multi-wave Design
...
Prop_data <- sample_strata(data = Prop_data,  strata = "new_strata", id = "id", design_data = design_df, design_strata = "strata", n_allocated = "n")
...
Multi-wave adaptive sampling using optimum allocation efficiently minimizes the variance of an estimate, and it is thus useful in epidemiological studies where one or more covariates is difficult to measure. `Optimall` streamlines the adaptive sampling workflow in R, providing users with a simple framework for study design and analysis.
