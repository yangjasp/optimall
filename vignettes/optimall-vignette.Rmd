...
Below is an example of how `optimum_allocation` could be called to optimally allocate 20 samples among `Species` in the `iris` dataset, minimizing the variance of `Sepal.Width`.
...
The data is obtained from electronic health records, which are known to be error-prone. In a perfect world, we would validate every observation in our sample, but auditing electronic health records is an expensive and difficult task. We determine that we can only reasonably afford to validate 750 out of the 10,335 child-mother pairs. For simplicity, suppose that we want to use these 750 samples to estimate the true population mean of maternal weight change during pregnancy with minimal error. This goal will be best accomplished through an adaptive, multi-wave sampling design.
...
For the purpose of this example, we suppose that we do not have access to this full dataset. Instead, we must sample from it to optimally estimate the mean of the true maternal weight changes.

#### *Phase-I*

During phase-I, we select all 10335 mother-child pairs to be sampled for the inexpensive covariates. Accordingly, our `phase1` dataset is 10335 x 4 with every column of the full `MatWgt_Sim_Full` data excluding the expensive true weight change.
...
We suspect that the true mean maternal weight change may vary with some of these inexpensive covariates, so we decide to split our population into 9 non-overlapping strata. Each stratum will be defined by a unique combination of child race and global percentile of maternal weight gain (≤25th, 25th - 75th, >75th). We can accomplish this quickly in `optimall` using the `split_strata` function. 
...
#### *Phase-IIa*
With the strata now defined by the inexpensive variables sampled in phase-I, we are ready to begin auditing patient records for validation of the true maternal weight changes. Without any validated data to define the optimum phase-IIa sample allocation, we decide to use proportional stratified sampling for the first 250 out of our 750 audits. That is, we want the proportion of samples randomly taken from each stratum to reflect its population proportion. Conveniently, `optimall` can select this random sample for us with `sample_strata`, we just have to make a "design" dataframe specifying the $n_h$ values first. 
...
```{r}
phase2b <- merge_strata(data = phase2b, 
                          strata = "new_strata",
