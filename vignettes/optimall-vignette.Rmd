...
The `optimall` package allows users to select between Neyman allocation, Wright Algorithm I, and Wright Algorithm II, and Wright Algorithm III in the `method` argument of the `optimum_allocation()` function. Wright Algorithm III is the most general, allowing for specification of exact lower and upper bounds on the stratum sizes. Algorithms I and II are equivalent to Algorithm III with lower bounds of 1 unit and 2 units per stratum respectively. In `optimall`, stratum sampling sizes for all Wright algorithms are also constrained from above at $N_h$, the population stratum size. 
