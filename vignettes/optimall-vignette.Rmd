...
`optimall` allows users to input phase-I data and iteratively allocate samples for subsequent waves with the function `allocate_wave`, which is demonstrated in the examples below. Along the way, the user may also want to split strata to make them more balanced. `optimall` makes this easy with the `split_strata` function.
