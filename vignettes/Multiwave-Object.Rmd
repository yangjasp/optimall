...
but this is overly complicated and potentially unstable. Instead, any slot of the multiwave object can be accessed using the function `get_data()`. Similarly, any slot can written using `set_data()`:
...
set_data(MySurvey, phase = NA, slot = "metadata") <- list(title = "Maternal Weight Survey")
...
set_data(MySurvey, phase = 1, slot = "metadata") <- list(data_dict = phase1_data_dictionary)
```
...
set_data(IrisSurvey, phase = 1, slot = "data") <-
...
```{r}
set_data(IrisSurvey, phase = 2, slot = "metadata") <-
  list(strata = "Species")
```
...
```{r}
set_data(IrisSurvey, phase = 2, wave = 1, slot = "sampled_data") <-
...
set_data(MySurvey, phase = 2, slot = "metadata") <- 
...
set_data(MySurvey, phase = 2, wave = 1, slot = "metadata") <- 
...
```{r}
#Design for Wave 1
set_data(MySurvey, phase = 2, wave = 1, slot = "design") <- data.frame(
...
set_data(MySurvey, phase = 2, wave = 1) <- 
