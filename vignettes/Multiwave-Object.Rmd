...
but this is overly complicated and potentially unstable. Instead, any slot of the multiwave object can be accessed using the function `mwget()`. Similarly, any slot can written using `mwset()`:
...
mwset(MySurvey, phase = NA, slot = "metadata") <- list(title = "Maternal Weight Survey")
...
mwset(MySurvey, phase = 1, slot = "data") <- phase1
...
mwset(MySurvey, phase = 1, slot = "metadata") <- list(data_dict = phase1_data_dictionary)
```
...
mwset(IrisSurvey, phase = 1, slot = "data") <-
...
```{r}
mwset(IrisSurvey, phase = 2, slot = "metadata") <-
  list(strata = "Species")
```
...
```{r}
mwget(IrisSurvey, phase = 2, wave = 1, slot = "design")
```
...
```{r}
mwget(IrisSurvey, phase = 2, wave = 1, slot = "samples")
```
...
```{r}
mwset(IrisSurvey, phase = 2, wave = 1, slot = "sampled_data") <-
  iris[iris$id %in% mwget(IrisSurvey,
                             phase = 2,
                             wave = 1,
                             slot = "samples"),
       c("id", "Sepal.Width")]
...
```{r}
head(mwget(IrisSurvey, phase = 2, wave = 1, slot = "data"))
```
...
mwset(MySurvey, phase = 2, wave = 1, slot = "metadata") <- 
...
```{r}
#Design for Wave 1
mwset(MySurvey, phase = 2, wave = 1, slot = "design") <- data.frame(
...
mwget(MySurvey, phase = 2, wave = 1, slot = "design")
```
...
head(mwget(MySurvey, phase = 2, wave = 1, slot = "samples"))
length(mwget(MySurvey, phase = 2, wave = 1, slot = "samples"))
...
ids_wave1 <- mwget(MySurvey, phase = 2, wave = 1, slot = "samples") 
...
mwset(MySurvey, phase = 2, wave = 1) <- 
  mwget(MySurvey, phase = 2, wave = 1) %>%
