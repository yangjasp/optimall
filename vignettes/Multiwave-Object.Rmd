...
#data
MatWgt_Sim <- read.csv("/Users/jasper/Desktop/Sim_matWgts.csv")
...
The multiwave object is an S4 object, which makes it slightly more complicated to work with in R. Fortunately, all of these potential complications will be dealt with in the package design and will be unseen by the user. Here is what the user will see:

<center>
<br />
![](/Users/jasper/Desktop/MultiwaveStructure.png){width=65%}
</center>
<br />
...
To access and write slots of the multiwave object, the user could use `@` and `$`:
...
### Call `optimall` functions with less arguments
...
### Example Wave Workflow Using `optimal` with A Multiwave Object
...
The first step of Phase 2, Wave 1 is to specify the metadata for both the phase and wave:
...
get_data(MySurvey, phase = 2, slot = "metadata") <- 
  list(description = "Phase 2 of Maternal Weight Survey in which we
       ...
       strata = "new_strata", #this is strata column in data
       id = "id",  #name of id column
       y = "mat_weight_true", #var for which to minimize variance
       strata2 = "strata", #strata column in designs
...
After the metadata is specified, we create a dataframe specifying the survey design. It contains one column holding the stratum names and one column specifying how many samples should be allocated to each stratum in Wave 1. Since we have not collected any validated data to base our stratum standard deviations on yet, we will use proportional sampling for the first 250 samples.
...
We can use this design to randomly select ids to sample from the previous wave (in this case the overall Phase 1 data) using `apply_multiwave` to apply the function `sample_strata` to the multiwave object. When applied to an object of class "Multiwave", `sample_strata` will find `"data1"` and `"data2"` in the object and will look in the wave, phase, then overall metadata for the other arguments to `"sample_strata"`. It will return the same multiwave object with a vector of ids to sample in the `"samples"` slot of the specified wave.

```{r}

# Get list of ids to sample
set.seed(456)
...
We can use these sample ids to get the Phase 2, Wave 1 data:
...
Then, we can use the function `merge_samples` merge the sampled data into our full dataframe. This function generates the dataframe for the `"data"` slot of the specified wave using the `"sampled_data"` and the data from the previous wave (Phase 1 data in this case).
...
All the slots for this wave are now filled, so it is complete, but we may want to add an indicator for which samples were already sampled to this wave 1 data. This will be useful for future waves and can be easily done with `dplyr` because the already sampled ids are all stored in the `"samples"` slot of each wave:
...
We can check to make sure everything that we have done so far looks good:
```{r, eval = FALSE}
multiwave_diagram(MySurvey)
```
<center>
