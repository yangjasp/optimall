...
load(here::here("data/Example_SimPhase1.rda"))
load(here::here("data/Example_SimPhase2a.rda"))
load(here::here("data/Example_SimPhase2b.rda"))


phase1 <- Example_SimPhase1
phase1$id <- seq(1:10335)
phase2a <- Example_SimPhase2a
phase2b <- Example_SimPhase2b
```
...
```{r Example 1}
data <- datasets::iris
optimum_allocation(data = data, strata = "Species", y = "Sepal.Width", nsample = 20, method = "WrightII")
```
...
design <- survey::twophase(id=list(~1,~1),strata=list(NULL,~new_strata),subset=~sample_indicator,data=full_data,method="simple")
fit<-survey::svycoxph(survival::Surv(child_age,event)~ mat_age+ mat_obesity+ wgt_change_validated + relevel(as.factor(mat_race),ref="White")+delivery_method+ nchildren,design=design)
summary(fit)
