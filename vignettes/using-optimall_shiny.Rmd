---
title: "Splitting Strata with optimall's R Shiny App"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{using-optimall_shiny}
...
The first step in using `optimall_shiny` is providing the application with the data. The data must be in the same format that the rest of `optimall`'s functions require, meaning that each row must correspond to one sampling unit. If you have already updated an existing dataset in your R workflow, you will have to save the updated dataframe to a .csv file in order to work with it in `optimall_shiny`. 
...
`optimall_shiny` displays the output dataframe of `optimum_allocation` or `allocate wave` based on the current inputs. After a split is confirmed and these inputs are reset, it will show an error in place of this dataframe until valid inputs are selected again. Displayed below this dataframe is a text list of the code that performs each of the confirmed splits in `optimall`.

<center>
<br />
![Alt text](../inst/shiny-app/optimall_shiny/Screenshots/Screenshot4.png){width=80%}
</center>
<br />
